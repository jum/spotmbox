// spot.go - a simple command line tool to convert a mailbox full of
// spot style mail message into an gpx xml or a json file.
//
// jum@anubis.han.de
...
var (
	mbox      *string = flag.String("mbox", "spot.mbox", "mailbox to read")
	messenger *string = flag.String("messenger", "No Rush", "Messenger to filter")
...
func main() {
	...
	for i, m := range mb {
		//debug("m %#v\n", m)
		//debug("text %#v\n", m.Text)
		w, err := ParseSpotMessage(m.FullHeaders, m.Text)
		if err != nil {
			panic(err.Error())
		}
		if w.Longitude == 0 && w.Latitude == 0 {
			continue
		}
		if w.Messenger != *messenger {
			continue
		}
		if len(w.Name) == 0 {
